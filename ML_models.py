...
def run_ml_tl_model(scaler, base_X_train, base_X_test, base_y_train, base_y_test, base_df, tgt_df):
    ...
    base_train_df, _ = get_samples(seed, base_pos_subjs, base_neg_subjs, max(1, int(5/3)), base_df)
    tgt_train_df, tgt_test_df = get_samples(seed, pos_subjs, neg_subjs, 10, tgt_df)
    ...
    all_metrics = evaluate_predictions(f'SVM', tgt_y_test, tgt_test_df, base_y_test, base_preds)
    ...
    return metrics_list, metrics_grouped, base_metrics


def run_ml_fstl_model(scaler, base_X_train, base_X_test, base_y_train, base_y_test, base_df, tgt_df):
    n_features = np.shape(base_X_train)[1]
    base_clf = SGDClassifier()
    base_clf.partial_fit(base_X_train, base_y_train, classes=np.unique(base_y_train))
