# https://github.com/Neuro-Logical/speech/blob/main/Cross_Lingual_Evaluation/Non_Interpretable_features/nested_cross_validation/multilingual/PCA_PLDA_EER_Classifier.py
import numpy as np
...
class PCA_PLDA_EER_Classifier(BaseEstimator, ClassifierMixin):
    def __init__(self, PCA_n=20, eer_threshold=0.5, scaler=None, pca=None, plda=None, en_stat=None, en_sets=None, normalize=0):
    ...
    def fit(self, features_train, train_lbs):
        ...
        self.en_sets = np.array(en_sgs, dtype="|O")
        ...
        va_sets = np.array(va_sgs, dtype="|O")
        va_s = np.array([None] * va_N)
        va_stat0 =np.array([[1.0]]* va_N)
        ...
        fpr, tpr, threshold = roc_curve(train_lbs_tmp, train_X, pos_label=1, drop_intermediate=False)
        fnr = 1 - tpr
...
def run_PCA_PLDA(X_train, X_test, y_train, y_test, test_df):
    ...
    all_metrics = evaluate_predictions('PCAPLDA', y_test, test_df)
